...
'use strict';
const VERSION = 'wind-field-overlay/v1.5';
const OPEN_METEO_ENDPOINT = 'https://api.open-meteo.com/v1/forecast';
const OPEN_METEO_MODEL = 'ecmwf_ifs';
const CACHE_PREFIX = 'storm-track-wind-field-v3:';
const CACHE_TTL_MS = 45 * 60 * 1000;
const STALE_CACHE_TTL_MS = 2 * 60 * 60 * 1000;
...
function samplingProfile(options = {}) {
const mobile = options.mobile === true;
const zoom = finite(options.zoom) ?? 4;
const close = zoom >= 5.5;
return Object.freeze({
baseRows: mobile ? 7 : 8,
baseCols: mobile ? 9 : 10,
basePadRatio: 0.22,
coreRadiusKm: close ? 320 : 460,
coreRows: close ? 11 : 9,
coreCols: close ? 11 : 9,
maxLocalPatches: close ? 1 : MAX_LOCAL_PATCHES
});
}
function snapStormCenter(center, stepDegrees = 0.25) {
const lat = finite(center?.lat);
const lon = finite(center?.lon);
const step = finite(stepDegrees);
if (lat === null || lon === null || step === null || !(step > 0)) return center;
const snap = value => Math.round(value / step) * step;
return { ...center, lat: snap(lat), lon: snap(lon) };
}
function isRateLimitError(error) {
return /^wind-http-429(?:[:]|$)/.test(error instanceof Error ? error.message : String(error || ''));
}
...
function readCachedGrid(signature, maxAgeMs = CACHE_TTL_MS) {
try {
...
if (cached) return { grid: cached, fromCache: true, stale: false };
}
try {
const grid = await fetchWindGrid(spec, options);
writeCachedGrid(signature, grid);
return { grid, fromCache: false, stale: false };
} catch (error) {
if (isRateLimitError(error)) {
const stale = readCachedGrid(signature, STALE_CACHE_TTL_MS);
if (stale) return { grid: stale, fromCache: true, stale: true, rateLimited: true };
}
throw error;
}
}
...
mobile: root.innerWidth <= 640,
zoom: map.getZoom?.()
});
return readVisibleStormCenters()
.slice(0, profile.maxLocalPatches)
.map(center => snapStormCenter(center))
.map(center => buildStormCenteredGrid(center, {
radiusKm: profile.coreRadiusKm,
rows: profile.coreRows,
cols: profile.coreCols
}))
.filter(Boolean);
}
...
const throttle = Date.now() < state.rateLimitUntil ? ' · 節流保護' : '';
hud.innerHTML = `ECMWF IFS · 10 m · ${formatValidTime(state.grid.validTime)} HKT${refinement}${throttle}<br><span style="color:#777">模式風場 · 內核仍受 IFS 解析度限制 · 非官方風圈</span> · <a href="https://open-meteo.com/" target="_blank" rel="noopener">Open-Meteo</a>`;
};
...
const now = Date.now();
if (!force && state.rateLimitUntil > now) {
const seconds = Math.max(1, Math.ceil((state.rateLimitUntil - now) / 1000));
setStatus(`API 節流保護中 · 約 ${seconds} 秒後再試`, state.grid ? 'ok' : 'error');
updateHud();
startAnimation();
return;
}
...
rows: profile.baseRows,
cols: profile.baseCols,
padRatio: profile.basePadRatio,
kind: 'base'
});
if (!baseSpec) {
...
&& now - state.lastFetchAt < CACHE_TTL_MS) {
positionCanvas();
seedParticles();
startAnimation();
return;
...
const localGrids = [];
let localRateLimited = baseLoaded.rateLimited === true;
let staleUsed = baseLoaded.stale === true;
if (baseLoaded.rateLimited) state.rateLimitUntil = Date.now() + RATE_LIMIT_BACKOFF_MS;
for (const spec of localSpecs) {
if (state.rateLimitUntil > Date.now()) break;
try {
const loaded = await loadGrid(spec, { force });
if (!state.enabled || serial !== state.fetchSerial) return;
localGrids.push(loaded.grid);
staleUsed = staleUsed || loaded.stale === true;
if (loaded.rateLimited) {
localRateLimited = true;
state.rateLimitUntil = Date.now() + RATE_LIMIT_BACKOFF_MS;
break;
}
} catch (error) {
if (isRateLimitError(error)) {
...
: '';
const cacheText = staleUsed ? ' · 沿用較早快取' : '';
const throttleText = localRateLimited ? ' · 已啟動 65 秒節流' : '';
setStatus(`已載入 · ${formatValidTime(state.grid.validTime)} HKT${refinementText}${cacheText}${throttleText}`, 'ok');
updateHud();
seedParticles();
startAnimation();
} catch (error) {
...
state.rateLimitUntil = Date.now() + RATE_LIMIT_BACKOFF_MS;
if (state.grid) {
setStatus('API 已達分鐘限制 · 保留現有風場，65 秒後再試', 'ok');
updateHud();
startAnimation();
return;
}
...
&& Date.now() - state.lastFetchAt < CACHE_TTL_MS) {
seedParticles();
startAnimation();
return;
}
...
rateLimitUntil: state.rateLimitUntil || null
})
});
}
