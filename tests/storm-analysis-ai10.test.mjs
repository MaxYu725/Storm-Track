...
}
{
  ...
  const health=await handleRequest(new Request('https://example.test/health'),{ANALYSIS_DB:{}},deps);const hb=await health.json();assert.equal(hb.deterministicAnalysisVersion,'storm-analysis-orchestration/v3');assert.equal(hb.analysisCacheVersion,'analysis-cache/v2');
}
